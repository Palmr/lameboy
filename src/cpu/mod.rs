...
impl<'c> CPU<'c> {
    ...
    pub fn cycle(&mut self) -> u8 {
        ...
        let duration = match op {
            ...
            0x78 => {println!("LD A, B"); load_r8_r8(self, &Reg8::A, &Reg8::B)},
            0x79 => {println!("LD A, C"); load_r8_r8(self, &Reg8::A, &Reg8::C)},
            0x7A => {println!("LD A, D"); load_r8_r8(self, &Reg8::A, &Reg8::D)},
            0x7B => {println!("LD A, E"); load_r8_r8(self, &Reg8::A, &Reg8::E)},
            0x7C => {println!("LD A, H"); load_r8_r8(self, &Reg8::A, &Reg8::H)},
            0x7D => {println!("LD A, L"); load_r8_r8(self, &Reg8::A, &Reg8::L)},
    ...
    fn imgui_display<'a>(&mut self, ui: &Ui<'a>, imgui_debug: &mut ImguiDebug) {
        ui.window(im_str!("CPU"))
            ...
            .build(|| {
                ...
                ui.text(im_str!(" A: 0x{:02X}   -  B: 0x{:02X}", self.registers.a, self.registers.b));
                ui.text(im_str!(" C: 0x{:02X}   -  D: 0x{:02X}", self.registers.c, self.registers.d));
